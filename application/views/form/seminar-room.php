...
<main class="container" ng-app="form" ng-controller="FormController">
    ...
    <form novalidate name="form" class="form-horizontal" action="/reservation/reserve/<?=$space?>" method="post">
        ...
        <p class="col-sm-offset-2">
            <span class="glyphicon glyphicon-exclamation-sign"></span> 세미나실은 하루에 최대 3시간까지 사용할 수 있습니다.
        </p>
        <div class="form-group">
            <label for="space" class="col-sm-2 control-label"> 장소 </label>
            <div class="col-sm-10">
                <label class="radio-inline">
                    <input type="radio" ng-model="space" name="space" value="seminar_room_1" required> 세미나실 1 (20명)
                </label>
                <label class="radio-inline">
                    <input type="radio" ng-model="space" name="space" value="seminar_room_2"> 세미나실 2 (30명)
                </label>
        ...
        <div class="form-group has-feedback" ng-class="{'has-error':form.time_from.$touched && form.time_from.$invalid || form.time_to.$touched && form.time_to.$invalid}">
            ...
            <div class="col-sm-4">
                <input ng-model="time_from" type="datetime-local" name="time_from" min="{{dateAfter(2) | date:'yyyy-MM-ddTHH:mm'}}" max="{{dateAfter(15, true) | date:'yyyy-MM-ddTHH:mm'}}" placeholder="{{dateAfter(2) | date:'yyyy-MM-ddTHH:mm'}}" class="form-control time_from" required>
            ...
            <div class="col-sm-4">
                <input ng-model="time_to" type="datetime-local" name="time_to" min="{{dateAfter(2) | date:'yyyy-MM-ddTHH:mm'}}" max="{{dateAfter(15, true) | date:'yyyy-MM-ddTHH:mm'}}" placeholder="{{dateAfter(15) | date:'yyyy-MM-ddTHH:mm'}}" class="form-control time_to" required>
            ...
            <span class="help-block col-sm-offset-2 col-sm-10" ng-show="form.time_from.$error.min || form.time_from.$error.max || form.time_to.$error.min || form.time_to.$error.max"> 현재 세미나실은 {{dateAfter(2) | date:'MM월dd'}}일에서 {{dateAfter(14) | date:'MM월dd'}}일까지 빌릴 수 있습니다. </span>
        </div>
