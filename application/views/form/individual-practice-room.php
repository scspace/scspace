...
<main class="container" ng-app="form" ng-controller="FormController">
    <form novalidate name="form" class="form-horizontal" action="/reservation/reserve/<?=$space?>" method="post">
        ...
        <div class="form-group has-feedback" ng-class="{'has-error':form.time_from.$touched && form.time_from.$invalid || form.time_to.$touched && form.time_to.$invalid || (isValidTime == 'invalid'),'has-warning':responseError == true,'has-success':validity}">
            ...
            <div class="col-sm-4">
                <input ng-model="time_to" ng-change="validity=false" step="900" type="datetime-local" name="time_to" min="{{dateAfter(1) | date:'yyyy-MM-ddTHH:mm'}}" max="{{dateAfter(14) | date:'yyyy-MM-ddTHH:mm'}}" placeholder="{{dateAfter(13) | date:'yyyy-MM-ddTHH:mm'}}" class="form-control time_to" required>
            ...
            <span class="help-block col-sm-offset-2 col-sm-10" ng-show="form.time_from.$error.min || form.time_from.$error.max || form.time_to.$error.min || form.time_to.$error.max"> 현재 개인연습실은 {{dateAfter(1) | date:'MM.dd'}}일에서 {{dateAfter(13) | date:'MM.dd'}}일까지 빌릴 수 있습니다. </span>
