...
class Space_mirae_hall extends CI_Driver {
    ...
    public function validate(){
        ...
        if ($post['usage_equipment'] == 'yes'){
            if ( ! $this->is_valid_start_time($from, $to, 10)) return '장비를 사용할 경우 미래홀은 적어도 예약 10일 전 오후 11시 59분까지 예약해야합니다.';
        } else {
            if ( ! $this->is_valid_start_time($from, $to, 2)) return '장비를 사용하지 않을 미래홀은 적어도 예약 2일 전 오후 11시 59분까지 예약해야합니다.';
        }

        if ( ! $this->is_valid_end_time($from, $to, 45)) return '미래홀은 예약 45일 전부터 예약할 수 있습니다.';
