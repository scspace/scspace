...
class Space_dance_studio extends CI_Driver {
    ...
    public function validate(){
        ...
        if ( ! $this->is_valid_start_time($from, $to, 1)) return '무예실은 적어도 예약 하루 전 오후 9시까지 예약해야합니다.';

        if ( ! $this->is_valid_end_time($from, $to, 14)) return '무예실은 예약 14일 전 오후 9시부터 예약할 수 있습니다.';
        ...
        if (count($days_covered) == 2) {
            ...
            if ( ! $this->is_shorter($from, $second_day, $max_duration)) return '무예실은 하루에 최대 2시간까지 사용할 수 있습니다.';

            $max_duration = 7200;
            // echo 'max_duration: '.$max_duration.' days_covered: '.$second_day;
            if ( ! $this->is_shorter($second_day, $to, $max_duration)) return '무예실은 하루에 최대 2시간까지 사용할 수 있습니다.';
            ...
            if ( ! $this->is_shorter($from, $to, $max_duration)) return '무예실은 하루에 최대 2시간까지 사용할 수 있습니다.';
        }
